...
class ClubController extends Controller
{
    ...
    public function getByUser(User $user)
    {
        ...
        if ($authUser->profile_type === 'superadmin' && (int) $authUser->id === (int) $user->id) {
            $query = Club::query()
                ->with(['clubClasses.investitureRequirements', 'carpetaClassActivations', 'staffAdventurers', 'localObjectives', 'adventurerYearlyApplications.signatures', 'adventurerQuarterlyReports', 'adventurerInductionRequests', 'pathfinderAnnualApplications.signatures', 'pathfinderMonthlyReports.attachments', 'church:id,pastor_email,pastor_name', 'district.association.union'])
                ->orderBy('club_name');

...
const assistedEnrollment = reactive({
    ...
    staff: { name: '', email: '', password: '', password_confirmation: '' },
    member: {
        ...
        program_year: 1, father_guardian_name: '', father_guardian_email: '', father_guardian_phone: '', mother_guardian_name: '', mother_guardian_email: '', mother_guardian_phone: '',
        city: '', state: '', zip: '', school: '', pickup_authorized_people: [], consent_acknowledged: false, photo_release: false,
        disabilities: '', medication_allergies: '', food_allergies: '', dietary_considerations: '', immunization_notes: '', current_medications: '',
        physician_name: '', physician_phone: '', emergency_contact_name: '', emergency_contact_phone: '', insurance_provider: '', insurance_number: '', parent_guardian_signature: '', signed_at: '',
        phone: '', address: '', email: '', emergency_contact_email: '', custom_fields_json: {}, is_sda: true, baptism_date: '', mark_insurance_paid: false, mark_enrollment_paid: false,
    },
})
const assistedTotalSteps = computed(() => assistedEnrollment.enrollment_type === 'staff_account' ? 2 : 4)
const isPathfinderEnrollmentClub = computed(() => ['pathfinders', 'temp_pathfinder'].includes(enrollmentSession.value?.club?.club_type))
const isMasterGuideEnrollmentClub = computed(() => enrollmentSession.value?.club?.club_type === 'master_guide')
...
async function submitAssistedEnrollment() {
    ...
    try {
        const member = { ...assistedEnrollment.member }
        if (!member.home_address && member.mailing_address) {
            member.home_address = member.mailing_address
        }
        if (typeof member.pickup_authorized_people === 'string') {
            member.pickup_authorized_people = member.pickup_authorized_people.split('\n').map((name) => name.trim()).filter(Boolean)
        }
        const { data } = await axios.post(route('club.settings.enrollment.assisted.store'), {
            ...
            staff: assistedEnrollment.staff,
            member,
        })
        enrollmentSession.value = data.data
        ...
        <div class="mx-auto max-w-5xl space-y-6">
            ...
            <template v-else>
                ...
                <section class="rounded-xl bg-white p-5 shadow-sm md:p-6">
                    ...
                    <p class="text-sm text-slate-500">{{ tr('Incluye padres aprobados por solicitud y cuentas de padre creadas por el director, junto con sus hijos registrados.', 'Includes approved parent requests and parent accounts created by the director, together with their registered children.') }}</p>
                    <div v-if="!enrollmentSession.enrolled_parents?.length && !enrollmentSession.assisted_members?.length" class="mt-4 rounded border border-dashed p-4 text-sm text-slate-500">{{ tr('Aún no hay hijos registrados.', 'There are no registered children yet.') }}</div>
                    ...
                    <div v-if="enrollmentSession.assisted_members?.length" class="rounded border border-sky-200 bg-sky-50 p-4"><div class="font-semibold text-sky-950">{{ tr('Miembros inscritos sin cuenta de padre', 'Members enrolled without a parent account') }}</div><div class="mt-3 space-y-2 border-l-2 border-sky-300 pl-4"><div v-for="child in enrollmentSession.assisted_members" :key="child.id" class="flex flex-wrap items-center gap-2 text-sm"><span class="font-medium">{{ child.name }}</span><span class="text-slate-600"> · {{ child.age ? `${child.age} ${tr('años', 'years')}` : tr('Edad no registrada', 'Age not recorded') }} · {{ child.club_name || enrollmentSession.club?.club_name }} · {{ child.class_name || tr('Sin clase asignada', 'No class assigned') }}</span><select v-if="!child.class_name && child.can_assign_class" :disabled="childClassAssignmentId === child.id" class="rounded border border-blue-300 bg-white px-2 py-1 text-sm text-blue-800 disabled:opacity-60" @change="assignChildClass(child, $event)"><option value="">{{ tr('Asignar clase…', 'Assign class…') }}</option><option v-for="clubClass in enrollmentSession.classes" :key="clubClass.id" :value="clubClass.id">{{ clubClass.name }}</option></select></div></div></div>
                </section>
            </template>
            ...
            <div v-if="showAssistedEnrollment" class="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 p-4">
                <div class="mx-auto my-6 w-full max-w-2xl rounded-xl bg-white p-5 shadow-xl sm:p-7">
                    ...
                    <div v-if="assistedStep === 1" class="mt-6 space-y-4"><p class="text-lg font-medium">{{ tr('¿Qué necesitas registrar?', 'What do you need to register?') }}</p><label class="flex cursor-pointer items-center gap-3 rounded border p-4"><input v-model="assistedEnrollment.enrollment_type" type="radio" value="parent_and_member"><span><strong>{{ tr('Padre/madre y miembro', 'Parent and member') }}</strong><small class="block text-slate-500">{{ tr('La cuenta se crea y aprueba inmediatamente.', 'The account is created and approved immediately.') }}</small></span></label><label class="flex cursor-pointer items-center gap-3 rounded border p-4"><input v-model="assistedEnrollment.enrollment_type" type="radio" value="member_only"><span><strong>{{ tr('Solo miembro', 'Member only') }}</strong><small class="block text-slate-500">{{ tr('Registra al menor sin crear una cuenta de padre.', 'Register the child without creating a parent account.') }}</small></span></label><label class="flex cursor-pointer items-center gap-3 rounded border p-4"><input v-model="assistedEnrollment.enrollment_type" type="radio" value="staff_account"><span><strong>{{ tr('Cuenta de personal', 'Staff account') }}</strong><small class="block text-slate-500">{{ tr('La cuenta queda en Solicitudes de personal para asignarle clase o tesorería antes de aprobarla.', 'The account appears in Staff requests so you can assign a class or treasury role before approval.') }}</small></span></label></div>

                    <div v-if="assistedStep === 2" class="mt-6 grid gap-4 sm:grid-cols-2"><template v-if="assistedEnrollment.enrollment_type === 'staff_account'"><div class="sm:col-span-2"><h3 class="text-lg font-semibold">{{ tr('Cuenta de personal', 'Staff account') }}</h3><p class="text-sm text-slate-500">{{ tr('El personal podrá completar su información de perfil al iniciar sesión.', 'Staff can complete their profile information after signing in.') }}</p></div><label>{{ tr('Nombre', 'Name') }}<input v-model="assistedEnrollment.staff.name" class="mt-1 w-full rounded border p-3" /></label><label>{{ tr('Correo', 'Email') }}<input v-model="assistedEnrollment.staff.email" type="email" class="mt-1 w-full rounded border p-3" /></label><label>{{ tr('Contraseña temporal', 'Temporary password') }}<PasswordInput v-model="assistedEnrollment.staff.password" autocomplete="new-password" input-class="p-3" /></label><label>{{ tr('Confirmar contraseña', 'Confirm password') }}<PasswordInput v-model="assistedEnrollment.staff.password_confirmation" autocomplete="new-password" input-class="p-3" /></label></template><template v-else-if="assistedEnrollment.enrollment_type === 'parent_and_member'"><div class="sm:col-span-2"><h3 class="text-lg font-semibold">{{ tr('Cuenta del padre o madre', 'Parent account') }}</h3></div><label>{{ tr('Nombre', 'Name') }}<input v-model="assistedEnrollment.parent.name" class="mt-1 w-full rounded border p-3" /></label><label>{{ tr('Correo', 'Email') }}<input v-model="assistedEnrollment.parent.email" type="email" class="mt-1 w-full rounded border p-3" /></label><label>{{ tr('Teléfono', 'Phone') }}<input v-model="assistedEnrollment.parent.phone" class="mt-1 w-full rounded border p-3" /></label><label>{{ tr('Contraseña temporal', 'Temporary password') }}<PasswordInput v-model="assistedEnrollment.parent.password" autocomplete="new-password" input-class="p-3" /></label><label class="sm:col-span-2">{{ tr('Confirmar contraseña', 'Confirm password') }}<PasswordInput v-model="assistedEnrollment.parent.password_confirmation" autocomplete="new-password" input-class="p-3" /></label></template><template v-else><div class="sm:col-span-2"><h3 class="text-lg font-semibold">{{ tr('Contacto del padre o madre', 'Parent contact') }}</h3></div><label>{{ tr('Nombre', 'Name') }}<input v-model="assistedEnrollment.member.parent_name" class="mt-1 w-full rounded border p-3" /></label><label>{{ tr('Teléfono', 'Phone') }}<input v-model="assistedEnrollment.member.parent_cell" class="mt-1 w-full rounded border p-3" /></label><label class="sm:col-span-2">{{ tr('Correo', 'Email') }}<input v-model="assistedEnrollment.member.email_address" type="email" class="mt-1 w-full rounded border p-3" /></label></template></div>

                    <div v-if="assistedStep === 3" class="mt-6 grid gap-4 sm:grid-cols-2"><div class="sm:col-span-2"><h3 class="text-lg font-semibold">{{ tr('Datos del menor', 'Child details') }}</h3></div><label class="sm:col-span-2">{{ tr('Nombre', 'Name') }}<input v-model="assistedEnrollment.member.applicant_name" class="mt-1 w-full rounded border p-3" /></label><template v-if="!isMasterGuideEnrollmentClub"><label>{{ tr('Fecha de nacimiento', 'Birthdate') }}<input v-model="assistedEnrollment.member.birthdate" type="date" class="mt-1 w-full rounded border p-3" /></label><label>{{ tr('Edad', 'Age') }}<input v-model="assistedEnrollment.member.age" type="number" class="mt-1 w-full rounded border p-3" /></label><label>{{ tr('Grado', 'Grade') }}<input v-model="assistedEnrollment.member.grade" class="mt-1 w-full rounded border p-3" /></label><label>{{ tr('Teléfono', 'Phone') }}<input v-model="assistedEnrollment.member.cell_number" class="mt-1 w-full rounded border p-3" /></label><label class="sm:col-span-2">{{ tr('Dirección', 'Address') }}<input v-model="assistedEnrollment.member.mailing_address" class="mt-1 w-full rounded border p-3" /></label><template v-if="isPathfinderEnrollmentClub"><label>{{ tr('Ciudad', 'City') }}<input v-model="assistedEnrollment.member.city" class="mt-1 w-full rounded border p-3" /></label><label>{{ tr('Estado', 'State') }}<input v-model="assistedEnrollment.member.state" class="mt-1 w-full rounded border p-3" /></label><label>{{ tr('Código postal', 'Zip') }}<input v-model="assistedEnrollment.member.zip" class="mt-1 w-full rounded border p-3" /></label><label>{{ tr('Escuela', 'School') }}<input v-model="assistedEnrollment.member.school" class="mt-1 w-full rounded border p-3" /></label></template><label class="sm:col-span-2">{{ tr('Contacto de emergencia', 'Emergency contact') }}<input v-model="assistedEnrollment.member.emergency_contact" class="mt-1 w-full rounded border p-3" /></label></template><template v-else><label>{{ tr('Teléfono', 'Phone') }}<input v-model="assistedEnrollment.member.phone" class="mt-1 w-full rounded border p-3" /></label><label>{{ tr('Año del programa', 'Program year') }}<select v-model="assistedEnrollment.member.program_year" class="mt-1 w-full rounded border p-3"><option :value="1">1</option><option :value="2">2</option></select></label><label class="sm:col-span-2">{{ tr('Dirección', 'Address') }}<input v-model="assistedEnrollment.member.address" class="mt-1 w-full rounded border p-3" /></label><label class="sm:col-span-2">{{ tr('Correo', 'Email') }}<input v-model="assistedEnrollment.member.email" type="email" class="mt-1 w-full rounded border p-3" /></label></template></div>

                    <div v-if="assistedStep === 4" class="mt-6 space-y-4"><h3 class="text-lg font-semibold">{{ tr('Información final', 'Final information') }}</h3><div v-if="isPathfinderEnrollmentClub" class="grid gap-4 rounded border border-blue-100 bg-blue-50 p-4 sm:grid-cols-2"><p class="sm:col-span-2 text-sm font-semibold text-blue-900">{{ tr('Datos de padres/tutores requeridos para Conquistadores', 'Parent/guardian details required for Pathfinders') }}</p><label>{{ tr('Nombre del padre/tutor', 'Father/guardian name') }}<input v-model="assistedEnrollment.member.father_guardian_name" class="mt-1 w-full rounded border p-3" /></label><label>{{ tr('Teléfono del padre/tutor', 'Father/guardian phone') }}<input v-model="assistedEnrollment.member.father_guardian_phone" class="mt-1 w-full rounded border p-3" /></label><label>{{ tr('Nombre de la madre/tutora', 'Mother/guardian name') }}<input v-model="assistedEnrollment.member.mother_guardian_name" class="mt-1 w-full rounded border p-3" /></label><label>{{ tr('Teléfono de la madre/tutora', 'Mother/guardian phone') }}<input v-model="assistedEnrollment.member.mother_guardian_phone" class="mt-1 w-full rounded border p-3" /></label></div><label class="block">{{ tr('Alergias', 'Allergies') }}<textarea v-model="assistedEnrollment.member.allergies" class="mt-1 w-full rounded border p-3"></textarea></label><label class="block">{{ tr('Restricciones físicas', 'Physical restrictions') }}<textarea v-model="assistedEnrollment.member.physical_restrictions" class="mt-1 w-full rounded border p-3"></textarea></label><label class="block">{{ tr('Historial médico', 'Health history') }}<textarea v-model="assistedEnrollment.member.health_history" class="mt-1 w-full rounded border p-3"></textarea></label><label v-if="!isPathfinderEnrollmentClub" class="block">{{ tr('Firma del padre/madre', 'Parent signature') }}<input v-model="assistedEnrollment.member.signature" class="mt-1 w-full rounded border p-3" /></label></div>
                    <div v-if="assistedStep === 4 && isPathfinderEnrollmentClub" class="mt-4 grid gap-4 sm:grid-cols-2"><label>{{ tr('Correo del padre/tutor', 'Father/guardian email') }}<input v-model="assistedEnrollment.member.father_guardian_email" type="email" class="mt-1 w-full rounded border p-3" /></label><label>{{ tr('Correo de la madre/tutora', 'Mother/guardian email') }}<input v-model="assistedEnrollment.member.mother_guardian_email" type="email" class="mt-1 w-full rounded border p-3" /></label><label class="sm:col-span-2">{{ tr('Personas autorizadas para recoger', 'Authorized pickup people') }}<textarea v-model="assistedEnrollment.member.pickup_authorized_people" class="mt-1 w-full rounded border p-3" placeholder="Una persona por línea"></textarea></label><label>{{ tr('Médico primario', 'Primary physician') }}<input v-model="assistedEnrollment.member.physician_name" class="mt-1 w-full rounded border p-3" /></label><label>{{ tr('Teléfono del médico', 'Physician phone') }}<input v-model="assistedEnrollment.member.physician_phone" class="mt-1 w-full rounded border p-3" /></label></div>
                    <div v-if="assistedStep === 4 && isMasterGuideEnrollmentClub" class="mt-4 grid gap-4 sm:grid-cols-2"><label>{{ tr('Contacto de emergencia', 'Emergency contact') }}<input v-model="assistedEnrollment.member.emergency_contact_name" class="mt-1 w-full rounded border p-3" /></label><label>{{ tr('Teléfono de emergencia', 'Emergency phone') }}<input v-model="assistedEnrollment.member.emergency_contact_phone" class="mt-1 w-full rounded border p-3" /></label><label class="sm:col-span-2">{{ tr('Correo de emergencia', 'Emergency email') }}<input v-model="assistedEnrollment.member.emergency_contact_email" type="email" class="mt-1 w-full rounded border p-3" /></label></div>
                    <div v-if="assistedStep === 4 && !isPathfinderEnrollmentClub && !isMasterGuideEnrollmentClub" class="mt-4 rounded border p-4"><p class="mb-2 text-sm font-semibold">{{ tr('Clase de investidura', 'Investiture class') }}</p><div class="flex flex-wrap gap-3"><label v-for="level in ['Little Lambs', 'Eager Beavers', 'Busy Bee', 'Sunbeam', 'Builder', 'Helping Hand']" :key="level" class="inline-flex items-center gap-2 text-sm"><input v-model="assistedEnrollment.member.investiture_classes" type="checkbox" :value="level">{{ level }}</label></div></div>
                    <div v-if="assistedStep === 4" class="mt-4 rounded border border-blue-100 bg-blue-50 p-4"><label class="flex items-center gap-2 text-sm font-medium"><input v-model="assistedEnrollment.member.is_sda" type="checkbox">{{ tr('Miembro bautizado de la Iglesia Adventista del Séptimo Día', 'Baptized Seventh-day Adventist member') }}</label><label v-if="assistedEnrollment.member.is_sda" class="mt-3 block text-sm">{{ tr('Fecha de bautismo (opcional)', 'Baptism date (optional)') }}<input v-model="assistedEnrollment.member.baptism_date" type="date" class="mt-1 w-full rounded border p-3" /></label></div>

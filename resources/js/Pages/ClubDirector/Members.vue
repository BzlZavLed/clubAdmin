...
const fetchMembers = async (clubId) => {
    try {
        ...
        if(members.value.length == 0){
            showToast('Club without members', 'error')
        }else{
...
<PathfinderLayout>
    <div class="p-8">
        ...
        <div class="max-w-xl mb-6">
            ...
            <select v-model="selectedClub" @change="() => {
    if (selectedClub) {
        memberForm.club_id = selectedClub.id
        memberForm.club_name = selectedClub.club_name
        memberForm.director_name = selectedClub.director_name
        memberForm.church_name = selectedClub.church_name
        ...
        <div v-if="selectedClub && selectedClub.club_type === 'adventurers'" class="max-w-5xl mx-auto">
            ...
            <transition name="fade">
                <div v-if="toast.show" :class="['fixed top-6 right-6 z-50 px-4 py-2 rounded shadow text-white', toast.type === 'success' ? 'bg-green-600' : 'bg-red-600']">
                    {{ toast.message }}
                </div>
            </transition>
            <MemberRegistrationModal :show="showRegistrationForm" :clubs="clubs" :selectedClub="selectedClub" @close="showRegistrationForm = false" @submitted="fetchMembers(selectedClub.id)" />
            <DeleteMemberModal :show="showDeleteModal" :memberId="deletingMember?.id" :memberName="deletingMember?.applicant_name" @cancel="showDeleteModal = false" @confirm="handleMemberDelete" />
        </div>
    </div>

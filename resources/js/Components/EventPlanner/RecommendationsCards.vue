...
const cards = computed(() => (props.items || []).map(item => {
    ...
    const name = meta.name || item.label || tr('Lugar', 'Place')
...
<template>
    <div v-if="cards.length" class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div v-for="(card, idx) in cards" :key="idx" class="bg-white border rounded-lg p-4 space-y-2">
            ...
            <div v-if="card.distance || card.eta" class="text-xs text-gray-500">
                <span v-if="card.distance">{{ tr('Distancia', 'Distance') }}: {{ card.distance }}</span>
                <span v-if="card.eta"> • {{ tr('ETA', 'ETA') }}: {{ card.eta }}</span>
            </div>
    ...
    <div v-else class="text-sm text-gray-500">{{ tr('Aún no hay recomendaciones.', 'No recommendations yet.') }}</div>
</template>

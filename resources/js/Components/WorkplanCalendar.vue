    ...
    <div class="bg-white shadow-sm rounded-lg p-4 border overflow-x-auto">
        ...
        <div v-if="isMobile" class="space-y-2 w-full">
            <div class="grid grid-cols-1 gap-2 w-full">
                <div v-for="(cell, idx) in weekCells" :key="idx" class="border rounded-lg p-3 bg-white">
                    <div class="flex items-center justify-between text-[12px] text-gray-700 mb-1">
                        <span class="font-semibold text-gray-900">{{ localizedDateLabel(cell.date) }}</span>
                        <button v-if="canAdd && !isReadOnly" class="text-blue-600 text-sm" @click="emit('add', cell.date)">+</button>
                    </div>
                    <div class="space-y-1">
                        <div
        ...
        <div v-else class="min-w-[720px]">
            ...
            <div class="grid grid-cols-7 gap-2">
                <div v-for="(cell, idx) in calendarCells" :key="idx" class="min-h-[95px] sm:min-h-[110px] border rounded-lg p-2 bg-white">
                    <div v-if="cell" class="flex flex-col h-full">
                        <div class="flex items-center justify-between text-[11px] text-gray-500 mb-1">
                            <span class="font-medium text-gray-800">{{ cell.label }}</span>
                            <button v-if="canAdd && !isReadOnly" class="text-blue-600" @click="emit('add', cell.date)">+</button>
                        </div>
                        <div class="space-y-1">
                            <div
                                v-for="ev in cell.events"

...
const removeAbility = (index) => form.award_instruction_abilities.splice(index, 1)
        ...
        <div class="bg-white rounded-lg shadow-xl w-full max-w-4xl p-6 max-h-[90vh] overflow-y-auto">
            ...
            <form @submit.prevent="onSubmit" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label>Address</label><input v-model="form.address" type="text"
                    ...
                    <div>
                        <label>Cell Phone</label><input v-model="form.cell_phone" type="text"
                            class="w-full p-2 border rounded" />
                    </div>
                    <div>
                    ...
                    <div>
                        <label>Church name</label><input v-model="form.church_name" type="church_name" readonly
                            class="w-full p-2 border rounded" />
                    </div>
                    <div>
                ...
                <div class="mb-4">
                    <label class="block mb-2">
                        Do you now have, or have you had any injury/sickness that might limit your involvement in
                    ...
                    <div v-if="form.has_health_limitation === 'yes'" class="mt-4">
                        ...
                        <textarea v-model="form.health_limitation_description" class="w-full p-2 border rounded"
                            rows="3"></textarea>
                    </div>
                </div>
                ...
                <div class="mb-4">
                    <label class="block mb-2">
                        List all experience (Pathfinders, scouting, Sabbath School, etc.) that might qualify you for
                        Adventurer leadership.
                    </label>

                    <div v-for="(experience, index) in form.experiences" :key="index"
                        class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-2">
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" v-model="experience.date" placeholder="e.g. 2020–2022"
                                class="w-full p-2 border rounded" />
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <table class="w-full text-sm border rounded">
                        ...
                        <tbody>
                            <tr v-for="(entry, index) in form.award_instruction_abilities" :key="index">
                                <td class="p-2">
                                    <input v-model="entry.name" type="text" class="w-full p-1 border rounded"
                                ...
                                <td class="p-2 text-center">
                                    <button type="button" class="text-red-600 hover:underline"
                                        @click="removeAbility(index)">Remove</button>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    ...
                    <p class="text-sm text-gray-500 mt-2">Circle: T = Capable of teaching, A = Able to assist, I =
                        Interested in learning</p>
                </div>
                <h3><b>Section 5 Unlawful conduct</b></h3>
                <div class="mt-4">
                    <label class="block mb-1">
                        Have you been accused, charged, or disciplined for any unlawful sexual conduct, child abuse,
                    ...
                    <div v-if="form.unlawful_sexual_conduct === 'yes'" class="mt-4">
                        <div class="mb-4">
                            ...
                            <div v-for="(entry, index) in form.unlawful_sexual_conduct_records" :key="index"
                                class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-2">
                                <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label>Pastor Reference</label><input v-model="form.reference_pastor" type="text"
                            class="w-full p-2 border rounded" />
                    </div>
                    <div>
                        <label>Elder Reference</label><input v-model="form.reference_elder" type="text"
                            class="w-full p-2 border rounded" />
                    </div>
                    <div>
                        <label>Other Reference</label><input v-model="form.reference_other" type="text"
                            class="w-full p-2 border rounded" />
                    </div>
                </div>
                <h3><b>Section 8 Statement of Accuracy</b></h3>
                <label>The above information is accurate to the best of my recollection. I understand this is strictly a
                    volunteer position, and I will receive no remuneration for services and time volunteered.</label>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label>Signature</label><input v-model="form.applicant_signature" type="text"
                            class="w-full p-2 border rounded" />
                    </div>
                    <div>
                        <label>Date</label><input v-model="form.application_signed_date" type="date"
                            class="w-full p-2 border rounded" />
                    </div>
                </div>
                ...
                <label>
                    <b>Notes</b>
                    Please make sure you have checked the appropriate box in Section 5 and signed your name in Section
                    8.

                    Directors are to mail the completed form to: Adventurer Ministries Conference Director.

                    Section 5 deals with unlawful conduct. This section has been included to protect the Adventurer Club
                    members from abuse and protect the Seventh-day Adventist church organization from recommending my
                    staff member who has a problem in this area.

                    If the conference Adventurer director recommends the applicant, information in Sections 1 through 4
                    will be copied and sent to the local Adventurer Club for the director to use in determining staff
                    qualification. If the applicant has not been approved, none of the information will be forwarded.

                    When a local club director requests a recommendation from the conference Adventurer director, he/she
                    may not release any specifics and may respond only with “recommended”, “not recommended”, or
                    “recommended with conditions noted”.

                    All information on this application will become a permanent record and should include updates. In
                    the event of accusations against the applicant, opportunity should be given for response by the
                    accused. This response also becomes a part of the record.

                    We regret having to include a section on unlawful conduct, however, understanding the epidemic
                    proportions of this problem, it becomes necessary to create a data base to protect child, parents,
                    Adventurer staff, and the church.
